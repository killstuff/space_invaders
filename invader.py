...
from pyglet.image import ImageGrid, load, Animation
...
class GameLayer(cocos.layer.Layer):
    ...
    def update(self, dt):
        ...
        if len(self.collman.known_objs()) == 1:
            self.hud.show_you_win()
...
class HUD(cocos.layer.Layer):
    ...
    def show_you_win(self):
        w, h = cocos.director.director.get_window_size()
        message = cocos.text.Label('YOU WIN!', font_size = 50,
                                     anchor_x = 'center',
                                     anchor_y = 'center')
        message.position = w * 0.5, h * 0.5
        self.add(message)
